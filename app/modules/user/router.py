...
from app.modules.user.models import UserRegisterModel, UserLoginModel, UserNamesModel, UserUpdateRoleModel, UserBaseModel, UserMailModel, TokenBase
...
async def register(user: UserRegisterModel, db: Annotated[Session, Depends(get_global_db)]) -> UserBaseModel:
    return user_crud.create_user(db, user)

@router.post("/login")
async def login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: Annotated[Session, Depends(get_global_db)]) -> TokenBase:
...
async def update_user_names(updated_user: UserNamesModel, current_user: Annotated[UserMailModel, Depends(get_current_user)], db: Annotated[Session, Depends(get_global_db)]) -> UserBaseModel:
    return user_crud.update_user_names(db, current_user, updated_user)

@router.put("/update/roles")
async def update_user_roles(updated_user: UserUpdateRoleModel, current_user: Annotated[UserMailModel, Security(get_current_user, scopes=["admin"])],  db: Annotated[Session, Depends(get_global_db)]) -> UserBaseModel:
